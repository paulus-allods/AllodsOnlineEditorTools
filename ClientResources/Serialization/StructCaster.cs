...
public class StructCaster(IReadOnlyDictionary<string, Type> sourceStructs, IReadOnlyDictionary<string, Type> targetStructs, ILogger logger)
{
    ...
    public void Analyze(IEnumerable<string> structNames)
    {
        ...
        {
            if (!_analyzedStructs.Add(name)) continue;
            
            if (!sourceStructs.TryGetValue(name, out var sourceType)) continue;

            if (!targetStructs.TryGetValue(name, out var targetType))
            {
                logger.LogWarning("Struct {Struct} has no implementation in the target version; its resources will be skipped", name);
    ...
    public object Cast(object source, ResourceSerializationContext? context = null)
    {
        ...
        {
            throw new InvalidOperationException($"No cast plan for '{source.GetType().Name}'; call {nameof(Analyze)} first and check {nameof(CanCast)}");
        }
    ...
    {
        if (_planCache.TryGetValue((sourceType, targetType), out var cached)) return cached;
        ...
        foreach (var sourceField in StructModelCache.Get(sourceType).Fields)
        {
            ...
            {
                logger.LogWarning("Field {Struct}.{Field} does not exist in the target version; it will be dropped", sourceType.Name, sourceField.Name);
                IncompatibilityCount++;
                continue;
            }
            ...
            if (!supported)
            {
                logger.LogWarning("Field {Struct}.{Field} cannot be cast from {SourceType} to {TargetType}; it will be dropped",
        ...
        foreach (var (xdbName, targetField) in targetFields)
        {
            ...
            {
                logger.LogWarning("Field {Struct}.{Field} does not exist in the source version; it will be left at its default value", targetType.Name, targetField.Name);
                IncompatibilityCount++;
    ...
    private (bool Supported, ConvertFunc? Convert) BuildArrayConverter(Type sourceType, Type targetType)
    {
        ...
        if (!supported || elementConvert is null) return (false, null);

        return (true, (value, ctx) =>
        {
            if (value is not Array sourceArray) return null;
    ...
    private (bool Supported, ConvertFunc? Convert) BuildEnumAwareConverter(StructField sourceField, StructField targetField)
    {
        ...
        if (sourceAttr?.UseSourceOnCast == true || targetAttr?.UseSourceOnCast == true)
        {
            ...
            {
                logger.LogWarning("Field {Struct}.{Field} uses the source enum on cast but the source version has none; raw numbers will be written",
        ...
        if (sourceEnum is null || targetEnum is null || sourceEnum == targetEnum) return (true, null);

        var valueMap = BuildEnumValueMap(sourceEnum, targetEnum);
        if (sourceField.FieldType == typeof(int))
        {
            return (true, (value, _) => value is int intValue ? RemapEnumValue(intValue, sourceEnum, targetEnum, valueMap) : value);
        }
        return (true, (value, _) =>
        {
            if (value is not int[] values) return value;
    ...
    {
        if (valueMap.TryGetValue(value, out var mapped)) return mapped;

        if (_reportedEnumMisses.TryAdd((sourceEnum, targetEnum, value), 0))
        {
            logger.LogWarning("Enum value {Value} ({Name}) of {SourceEnum} has no counterpart in {TargetEnum}; the numeric value is kept",
