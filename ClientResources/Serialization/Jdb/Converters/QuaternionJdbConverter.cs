...
internal class QuaternionJdbConverter : JdbConverter<Quaternion>
{
    ...
    protected override Quaternion ReadValue(JdbStructSerializer serializer, JsonElement element, Type type) => new(element.GetProperty("x").GetSingle(),
        element.GetProperty("y").GetSingle(), element.GetProperty("z").GetSingle(), element.GetProperty("w").GetSingle());
}
