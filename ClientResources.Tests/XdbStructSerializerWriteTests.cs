...
public class XdbStructSerializerWriteTests
{
    ...
    [Test]
    public void Double_SerializesAsText()
        => Assert.That(Xml(Field(2.5d, "weight", typeof(double))), Is.EqualTo("<weight>2.5</weight>"));
    ...
    [Test]
    public void EmptyString_SerializesAsEmptyElement()
        => Assert.That(Xml(Field("", "name", typeof(string))), Is.EqualTo("<name />"));

    [Test]
    public void NullString_SerializesAsEmptyElement()
        => Assert.That(Xml(Field(null, "name", typeof(string))), Is.EqualTo("<name />"));
    ...
    public void EnumRefInt_SerializesMemberName()
        => Assert.That(Xml(Serializer.SerializeObject(new EnumRefHolder(), "Root").Element("kind")),
            Is.EqualTo("<kind>CREATURE_KIND_HORIZONTAL</kind>"));

    // Values missing from the (incomplete) recovered enum fall back to the raw number.
    [Test]
    public void EnumRefInt_UnknownValue_SerializesRawNumber()
        => Assert.That(Xml(Serializer.SerializeObject(new EnumRefHolder(), "Root").Element("unknownKind")),
            Is.EqualTo("<unknownKind>42</unknownKind>"));
    ...
    [Test]
    public void PrimitiveArray_WrapsElementsInItem()
        => Assert.That(Xml(Field(new[] { 0, 1, 2 }, "shaderIndices", typeof(int[]))),
    ...
    [Test]
    public void StringArray_WrapsElementsInItem()
    ...
    [Test]
    public void NullArray_SerializesAsEmptyElement()
        => Assert.That(Xml(Field(null, "objects", typeof(int[]))), Is.EqualTo("<objects />"));
    ...
    [Test]
    public void EmptyFileRef_SerializesAsEmptyHref()
        => Assert.That(Xml(Field(new FileRef(""), "binaryFile", typeof(FileRef))),
    ...
    [Test]
    public void EmptyTextFileRef_SerializesAsEmptyHref()
        => Assert.That(Xml(Field(new TextFileRef(""), "description", typeof(TextFileRef))),
    ...
    [Test]
    public void NullablePointer_SerializesConcreteTypeAndFields()
        => Assert.That(Xml(Field(new NullablePointer(new SampleParams()), "params", typeof(NullablePointer))),
    ...
    [Test]
    public void NullNullablePointer_IsSkipped()
        => Assert.That(Field(NullablePointer.Empty, "params", typeof(NullablePointer)), Is.Null);

    // ---------------------------------------------------------------- Math types

    [Test]
    public void Vector2_SerializesAsXyAttributes()
        => Assert.That(Xml(Field(new Vector2(1, 2), "uv", typeof(Vector2))),
            Is.EqualTo("<uv x=\"1\" y=\"2\" />"));

    // Sample: <center x="0.06831549" y="-0.260649949" z="0.693154752" /> (Geometry.xdb)
    [Test]
    public void Vector3_SerializesAsXyzAttributes()
        => Assert.That(Xml(Field(new Vector3(1, 2, 3), "center", typeof(Vector3))),
            Is.EqualTo("<center x=\"1\" y=\"2\" z=\"3\" />"));

    [Test]
    public void Quaternion_SerializesAsAttributes()
        => Assert.That(Xml(Field(new Quaternion(0, 0, 0, 1), "rotation", typeof(Quaternion))),
            Is.EqualTo("<rotation x=\"0\" y=\"0\" z=\"0\" w=\"1\" />"));

    [Test]
    public void BigVector3_SerializesAsXyzAttributes()
        => Assert.That(Xml(Field(new BigVector3(0, 0, 0, 1, 2, 3), "position", typeof(BigVector3))),
    ...
    {
        var xml = XElement.Parse(Serializer.SerializeResource(new SampleResource(), 564002825))
            .ToString(SaveOptions.DisableFormatting);
        Assert.That(xml, Is.EqualTo(
            "<gameMechanics.constructor.schemes.item.VisualItem>" +
            "<Header><resourceId>564002825</resourceId></Header>" +
            "<dressSlot>3</dressSlot>" +
            "</gameMechanics.constructor.schemes.item.VisualItem>"));
    }
    ...
    {
        var xml = XElement.Parse(Serializer.SerializeResource(new SampleResource(), 0))
            .ToString(SaveOptions.DisableFormatting);
        Assert.That(xml, Is.EqualTo(
            "<gameMechanics.constructor.schemes.item.VisualItem>" +
            "<dressSlot>3</dressSlot>" +
